...
SidWiz.prototype = {
	...
	draw: function(data, scaleIdx) {
		if (typeof window._gPlayerAudioCtx == 'undefined') return;	// not ready yet
...
class ScopeDisplay {
	constructor(tracer, canvId1, width, height) {
		this.tracer = tracer;
		this.sidWizMode= true;
		this.zoom= 3;	// lower is more zoom
		
		this.setZoom(this.zoom);
		
		this.vd = new VoiceDisplay(canvId1, function() { return tracer.getData(0);}, false, width, height);
	}
	reqAnimationFrame() {
		window.requestAnimationFrame(this.redraw.bind(this));
	}
	redraw() {
		this.vd.redrawGraph(this.sidWizMode, this.zoom);	
		this.reqAnimationFrame();	
	}
	setZoom(i) {
		this.zoom= i;
		if ( !this.sidWizMode) {
			this.tracer.setOutputSize(246 << i);
		} else {
			this.tracer.setOutputSize(16384);	// always use max XXX why MAX?
		}
	}	
};
